...
public: 
    ...
    // User-side API
    void ExtractInfomationFromScanForLoopDetect( pcl::PointCloud<SCPointType> & _scan_down );
...
private:
    ...
    const double SC_DIST_THRES = 0.2; // 0.4-0.6 is good choice for using with robust kernel (e.g., Cauchy, DCS) + icp fitness threshold / if not, recommend 0.1-0.15
