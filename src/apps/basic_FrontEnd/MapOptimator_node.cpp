...
const int windows_size = 25;              // 滑动窗口尺寸  
...
void laserOdometryHandler(const nav_msgs::Odometry::ConstPtr &laserOdometry)
{
	...
	br.sendTransform(tf::StampedTransform(transform, odomAftMapped.header.stamp, odom_frame_id, lidar_frame_id));
}
  ...
  { 
    ...
    {  
       ...
       if(full)
       {
        ...
        {
        //    ROS_INFO_STREAM("low moving x:"<<dx<<" da:"<<da);
            break;
        }    
...
//       ROS_INFO_STREAM("frame size: "<<laserCloudNum);        
...
//        keyframe_broadcaster.sendTransform(keyframe_trans);
        ...
        laserCloud.header.frame_id = lidar_frame_id;
